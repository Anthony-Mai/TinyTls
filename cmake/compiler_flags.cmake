if(COMPILER_FLAGS_CMAKE_)
  return()
endif() # COMPILER_FLAGS_CMAKE_
set(COMPILER_FLAGS_CMAKE_ 1)
...
set(MY_FAILED_C_FLAGS)
set(MY_FAILED_CXX_FLAGS)
...
# $CMAKE_C_FLAGS_<CONFIG>s stored in MY_C_CONFIGS when the compile test passes.
# Caches $c_flag in $MY_C_FLAGS or $MY_FAILED_C_FLAGS depending on test
# outcome.
function(add_c_flag_if_supported c_flag)
  ...
  is_flag_present(MY_C_FLAGS "${c_flag}" flag_ok)
  is_flag_present(MY_FAILED_C_FLAGS "${c_flag}" flag_failed)
  ...
  if(${C_FLAG_SUPPORTED})
    append_flag(MY_C_FLAGS "${c_flag}")
    foreach(config ${MY_C_CONFIGS})
      unset(C_FLAG_FOUND)
      append_flag("${config}" "${c_flag}")
    endforeach()
  else()
    append_flag(MY_FAILED_C_FLAGS "${c_flag}")
  endif()
endfunction()

# Checks C++ compiler for support of $cxx_flag. Adds $cxx_flag to all
# $CMAKE_CXX_FLAGS_<CONFIG>s stored in MY_CXX_CONFIGS when the compile test
# passes. Caches $cxx_flag in $MY_CXX_FLAGS or $MY_FAILED_CXX_FLAGS depending
# on test outcome.
function(add_cxx_flag_if_supported cxx_flag)
  ...
  is_flag_present(MY_CXX_FLAGS "${cxx_flag}" flag_ok)
  is_flag_present(MY_FAILED_CXX_FLAGS "${cxx_flag}" flag_failed)
  if(${flag_ok} OR ${flag_failed})
    return()
  endif()
  ...
  if(${CXX_FLAG_SUPPORTED})
    append_flag(MY_CXX_FLAGS "${cxx_flag}")
    foreach(config ${MY_CXX_CONFIGS})
      unset(CXX_FLAG_FOUND)
      append_flag("${config}" "${cxx_flag}")
    endforeach()
  else()
    append_flag(MY_FAILED_CXX_FLAGS "${cxx_flag}")
  endif()
endfunction()
...
function(require_c_flag c_flag update_c_flags)
  ...
  if(NOT "${MY_EXE_LINKER_FLAGS}" STREQUAL "")
    push_var(CMAKE_EXE_LINKER_FLAGS "${MY_EXE_LINKER_FLAGS}")
  endif()
  ...
  if(NOT "${MY_EXE_LINKER_FLAGS}" STREQUAL "")
    pop_var(CMAKE_EXE_LINKER_FLAGS)
  endif()
...
function(add_c_preproc_definition preproc_def)
  ...
  is_flag_present(MY_C_FLAGS "${preproc_def}" flag_cached)
  if(${flag_cached})
    return()
  endif()
...
function(add_cxx_preproc_definition preproc_def)
  set(preproc_def "-D${preproc_def}")
  is_flag_present(MY_CXX_FLAGS "${preproc_def}" flag_cached)
  if(${flag_cached})
    return()
  endif()
...
function(append_as_flag flag)
  is_flag_present(MY_AS_FLAGS "${flag}" flag_cached)
  if(${flag_cached})
    return()
  endif()
  append_flag(MY_AS_FLAGS "${flag}")
endfunction()

# Adds $flag to the C compiler command line.
function(append_c_flag flag)
  is_flag_present(MY_C_FLAGS "${flag}" flag_cached)
  if(${flag_cached})
    return()
  endif()
...
function(append_cxx_flag flag)
  is_flag_present(MY_CXX_FLAGS "${flag}" flag_cached)
  if(${flag_cached})
    return()
  endif()
...
function(append_exe_linker_flag flag)
  is_flag_present(MY_EXE_LINKER_FLAGS "${flag}" flag_cached)
  if(${flag_cached})
    return()
  endif()

  append_flag(MY_EXE_LINKER_FLAGS "${flag}")
  foreach(config ${EXE_LINKER_CONFIGS})
    append_flag(${config} "${flag}")
  endforeach()
endfunction()
...
function(require_linker_flag flag)
  ...
  unset(c_passed)
  check_c_compiles("LINKER_FLAG_C_TEST(${flag})" "" c_passed)
  unset(cxx_passed)
  check_cxx_compiles("LINKER_FLAG_CXX_TEST(${flag})" "" cxx_passed)
...
# Appends flags in $EXTRA_<TYPE>_FLAGS variables to the flags used at build
# time.
function(set_user_flags)
  ...
  if(EXTRA_EXE_LINKER_FLAGS)
    is_flag_present(MY_EXE_LINKER_FLAGS "${EXTRA_EXE_LINKER_FLAGS}"
                    extra_present)
    if(NOT ${extra_present})
      require_linker_flag("${EXTRA_EXE_LINKER_FLAGS}")
    endif()
  endif()
  if(EXTRA_AS_FLAGS)

    # TODO: assembler flag testing would be a good thing to have.
    is_flag_present(MY_AS_FLAGS "${EXTRA_AS_FLAGS}" extra_present)
    if(NOT ${extra_present})
      append_flag(MY_AS_FLAGS "${EXTRA_AS_FLAGS}")
    endif()
  endif()
  if(EXTRA_C_FLAGS)
    is_flag_present(MY_C_FLAGS "${EXTRA_C_FLAGS}" extra_present)
    if(NOT ${extra_present})
      require_c_flag("${EXTRA_C_FLAGS}" YES)
    endif()
  endif()
  if(EXTRA_CXX_FLAGS)
    is_flag_present(MY_CXX_FLAGS "${EXTRA_CXX_FLAGS}" extra_present)
    if(NOT ${extra_present})
      require_cxx_flag("${EXTRA_CXX_FLAGS}" YES)
    endif()
  endif()
endfunction()
